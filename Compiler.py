...
def compile_python_stealth(file_path, one_file=True, console=False, icon=None, name=None,
        ...
        if add_decoys or encrypt_strings:
            ...
            if add_decoys:
                ...
                for _ in range(3):
                    ...
                    decoy_functions.append(f"""def {func_name}():
    ...
    return {generate_random_name()} if {random.choice([True, False])} else {random.randint(1, 1000)}""")
            ...
            # Add base64 import if string encryption is enabled
            if encrypt_strings and "import base64" not in modified_code:
        ...
        # Add console flag based on parameter
        if not console:
        ...
        runtime_hook_path = create_runtime_hook(temp_dir)
